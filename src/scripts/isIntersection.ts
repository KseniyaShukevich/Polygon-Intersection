...
function checkPoint(
    ...
    const inRangeX: boolean = xCross <= maxX && xCross >= minX;
    const inRangeZ: boolean = zCross <= maxZ && zCross >= minZ; 

    return inRangeX && inRangeZ;
}
...
function checkSuperimposition(
    ...
    const isSamePointZ1: boolean = line.point1.z === z1 || line.point1.z === z2;
    const isSamePointZ2: boolean = line.point2.z === z1 || line.point2.z === z2; 

    return isSamePointZ1 || isSamePointZ2;
}
...
function checkLinesAlongOrdinates(
    ...
    const isLineSuperimposition1: boolean = checkSuperimposition(line1, minZ1, minZ2);
    const isLineSuperimposition2: boolean = checkSuperimposition(line2, minZ1, minZ2);

    return isLineSuperimposition1 && isLineSuperimposition2;
}
...
export default function isIntersection(
    ...
    const inLine1: boolean = checkPoint(line1, xCross, zCross);
    const inLine2: boolean = checkPoint(line2, xCross, zCross);
